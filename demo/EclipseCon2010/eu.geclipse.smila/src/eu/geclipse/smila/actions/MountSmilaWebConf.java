...
public class MountSmilaWebConf extends AbstractAWSProjectAction {
		...
		if (action.isEnabled()) {
			...
			Job job = new Job("Mount a Grid connection to the instance") { //$NON-NLS-1$
				...
				protected IStatus run(final IProgressMonitor monitor) {
					try {
						if (MountSmilaWebConf.this.instanceList.size() == 0) {
							return Status.CANCEL_STATUS;
						}

						for (EC2Instance instance : MountSmilaWebConf.this.instanceList) {

							// extract the hostname from the instance

							String hostname = instance.getHostName();

							String connectionname = "web.xml-" + hostname;
							...
							try {
								...
								URI uri = new URI("sftp", "root", hostname, 22,
										"/smila/configuration/org.eclipse.smila.connectivity.framework/web.xml", null, null);
