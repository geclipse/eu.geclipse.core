...
package eu.geclipse.aws.test.util;
 ...
 * Unit Tests. Especially generation of {@link AWSVoCreator}s and
 * {@link AWSVirtualOrganization}s.
 * 
 * @author Moritz Post
 */
public class AWSVoTestUtil {
  ...
  /**
   * Fetches the {@link AWSVoCreator} from the GridModel and returns it. If no
  ...
  public static AWSVoCreator getAwsVoCreator() {
    return ( AWSVoCreator )GridModel.getCreatorRegistry()
      .getCreator( null, AWSVirtualOrganization.class );
    /*
     * List<IGridElementCreator> voCreators = GridModel.getVoCreators();
     * AWSVoCreator awsVoCreator = null; for( IGridElementCreator
     * gridElementCreator : voCreators ) { if( gridElementCreator instanceof
     * AWSVoCreator ) { awsVoCreator = ( AWSVoCreator )gridElementCreator; } }
     * if( awsVoCreator != null ) { awsVoCreator.setVoName(
     * AWSVoTestUtil.VO_NAME ); awsVoCreator.setAwsAccessId(
     * AWSVoTestUtil.AWS_ACCESS_ID ); } return awsVoCreator;
     */
  }

  /**
   * Creates a new {@link AWSVirtualOrganization} with the property values
   * {@link #VO_NAME}.
   * 
