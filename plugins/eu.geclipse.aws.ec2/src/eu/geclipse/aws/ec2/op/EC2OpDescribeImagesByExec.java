 ...
 * @author Moritz Post
 */
public class EC2OpDescribeImagesByExec extends AbstractEC2OpDescribeImages {
  ...
  /**
   * Creates a new {@link EC2OpDescribeImagesByExec} with the given owners as
   * parameter.
   * 
   * @param ec2Service the {@link IEC2} to obtain data from
   * @param ownerArray the list of people whos AMIs to fetch
   */
  public EC2OpDescribeImagesByExec( final IEC2 ec2Service,
  ...
  public void run() {
    ...
    setException( null );
    try {
      setResult( this.ec2Service.describeImagesByExec( this.ownerList ) );
    } catch( Exception ex ) {
      setException( ex );
    }
