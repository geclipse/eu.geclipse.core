...
/**
 * This {@link IOperation} disassociates a currently bound elastic IP with a
 * running EC2 instance.
 * 
 * @author Moritz Post
 */
public class EC2OpDisassociateAddress implements IOperation {
   ...
   * interfacing gateway and the elastic IP to disassociate from a running
   * instance.
   * 
   * @param ec2 the gateway to ec2
   * @param elasticIP the IP address to release
   */
  public EC2OpDisassociateAddress( final IEC2 ec2, final String elasticIP ) {
    this.ec2Service = ec2;
  ...
  public void run() {
    this.exception = null;
    try {
      this.ec2Service.disassociateAddress( this.elasticIP );
    } catch( Exception ex ) {
      this.exception = ex;
    }
  }
